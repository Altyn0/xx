...
int main()
{
    ...
    do
    {
        ...
        case 2:
        {
            ...
            { // Lặp qua từng số trong khoảng [min, max]
                if (x < 2)
                    continue; // Bỏ qua các số nhỏ hơn 2, vì chúng không phải số nguyên tố

                int count = 0; // Biến đếm số ước của `x`
                for (int i = 2; i < x; i++)
                { // Kiểm tra ước số của `x`
                    if (x % i == 0)
                    { // Nếu `x` chia hết cho `i`, tăng `count`
                        count++;
                ...
                if (count == 0)
                { // Nếu không có ước nào, `x` là số nguyên tố
                    printf("%d ", x);
        ...
        case 3:
        {
            ...
            {                  // Lặp qua từng số trong khoảng [min, max]
                ...
                for (int i = 1; i * i <= x; i++)
                { // Kiểm tra từ 1 đến căn bậc hai của x
                    if (i * i == x)
                    { // Nếu i * i bằng x, x là số chính phương
                        printf("%d ", x);
            ...
            if (!found)
            { // Nếu không tìm thấy số chính phương nào
